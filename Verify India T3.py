...
class CroplandProcessor:
    ...
    def Pull_Process_Sentinel_data(self, QA_BAND='cs_cdf', CLEAR_THRESHOLD=0.50):
        """
        Process Sentinel-2 data with optimized masking for maximum image capture
        Very relaxed thresholds to capture as many images as possible
        """
        
        try:
            def mask_clouds_minimal(img):
                """
                Minimal cloud masking - only remove the most problematic pixels
                This allows maximum image retention while still filtering obvious issues
                """
                try:
                    # Use cloud score but with very relaxed threshold
                    cs = img.select(QA_BAND)
                    cloud_mask = cs.gte(CLEAR_THRESHOLD)  # 0.50 - very permissive
                    
                    scl = img.select('SCL')
                    
                    # Only mask the absolute worst quality pixels:
            ...
            # Very relaxed cloud filtering - allow up to 90% cloud cover
            ...
            # Very low threshold for valid pixels
            final_collection = ndti_collection_with_counts.filter(
                ee.Filter.gt('valid_pixel_count', 5))
...
def main_bihar_analysis():
    ...
    print("🌾 Bihar Region NDTI Analysis (POLYGON-BASED)")
    print("=" * 60)
    print(f"📍 Region: Bihar, India (Actual State Boundary Polygon)")
    print(f"📅 Period: {start_date} to {end_date} (6-Month Dry Season)")
    ...
    print()
    print("🔧 MAXIMUM IMAGE CAPTURE OPTIMIZATIONS:")
    print("   • Extended time: Oct 1, 2017 - Apr 1, 2018 (6 months)")
    print("   • Cloud threshold: 90%")
    print("   • Clear sky threshold: 0.50")
    print("   • Valid pixel minimum: 5 pixels")
    print("   • Minimal masking: Only saturated, high clouds, cirrus")
    print()
    print("📊 Current Ground Truth Deltas:")
    print("   • ΔMean: 0.039 | ΔMin: 0.065 | ΔMax: 0.069")
    print("   • Target: 200+ images for better accuracy")
    print()
...
if __name__ == "__main__":
    try:
        print("🚀 Starting Bihar Cropland NDTI Analysis (POLYGON-BASED)")
        print("📍 Region: Bihar, India (Actual State Boundaries)")
        print("📅 Period: October 2017 - April 2018 (6-Month Dry Season)")
        print("🔧 Mode: MAXIMUM Image Capture with Polygon Geometry")
        print("🎯 Goal: 200+ images | Current Deltas: Mean=0.039, Min=0.065, Max=0.069")
        print()
