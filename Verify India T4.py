...
class CroplandProcessor:
    ...
    def Pull_Process_Sentinel_data(self, QA_BAND='cs_cdf', CLEAR_THRESHOLD=0.65):
        """
        Process Sentinel-2 data with balanced masking
        Stricter thresholds for better quality while still capturing sufficient images
        """
        
        try:
            def mask_clouds_balanced(img):
                """
                Balanced cloud masking - stricter quality control
                Removes problematic pixels while keeping good data
                """
                try:
                    # Use cloud score with moderate threshold
                    cs = img.select(QA_BAND)
                    cloud_mask = cs.gte(CLEAR_THRESHOLD)  # 0.65 - moderate quality
                    ...
                    cloud_medium_mask = scl.neq(8)   # Medium probability clouds
                    cloud_high_mask = scl.neq(9)     # High probability clouds
                    cirrus_mask = scl.neq(10)        # Cirrus
                    
                    # Keep: dark areas (2), water (6), snow (11)
            ...
            # Stricter cloud filtering - better quality images
            filtered_s2_date_area = filtered_s2_date_area.filter(ee.Filter.lt('CLOUDY_PIXEL_PERCENTAGE', 70))
            ...
            # Moderate threshold for valid pixels
            final_collection = ndti_collection_with_counts.filter(
                ee.Filter.gt('valid_pixel_count', 20))  # Increased from 5 to 20
...
def main_bihar_analysis():
    ...
    print("🌾 Bihar Region NDTI Analysis (OPTIMIZED QUALITY)")
    print("=" * 60)
    ...
    print("   • More accurate than rectangular bounding box")
    print()
    print("🔧 BALANCED IMAGE QUALITY OPTIMIZATIONS:")
    print("   • Time: Oct 31, 2017 - Mar 31, 2018 (5 months core dry)")
    print("   • Cloud threshold: 70% (was 90%)")
    print("   • Clear sky threshold: 0.65 (was 0.50)")
    print("   • Valid pixel minimum: 20 pixels (was 5)")
    print("   • Balanced masking: Saturated, shadows, medium/high clouds, cirrus")
    print("   • KEEPS: dark areas, water, snow")
    print()
    print("📊 Previous Results (Improved!):")
    print("   • ΔMean: 0.045 | ΔMin: 0.050 | ΔMax: 0.043")
    print("   • Goal: Further reduce deltas with better quality images")
    print()
...
if __name__ == "__main__":
    try:
        print("🚀 Starting Bihar Cropland NDTI Analysis (QUALITY OPTIMIZED)")
        print("📍 Region: Bihar, India (Actual State Boundaries)")
        print("📅 Period: Oct 31, 2017 - Mar 31, 2018 (Core Dry Season)")
        print("🔧 Mode: Balanced Quality - Stricter Filtering")
        print("🎯 Previous: ΔMean=0.045, ΔMin=0.050, ΔMax=0.043")
        print()
